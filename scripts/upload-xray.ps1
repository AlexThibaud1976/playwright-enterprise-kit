<#
.SYNOPSIS
    Upload JUnit test results to Xray Cloud

.DESCRIPTION
    This script authenticates against the Xray Cloud API, then uploads a
    JUnit XML file to create a Test Execution in Jira/Xray.

    Handles:
    - Authentication via Xray Cloud client_id/client_secret
    - Upload of xray-report.xml generated by @xray-app/playwright-junit-reporter
    - Association with an existing Jira Test Plan
    - Export of the created Test Execution key (exec_key) to GITHUB_OUTPUT

.PARAMETER IssueKey
    Jira Test Plan key (e.g. MYPROJECT-100)

.PARAMETER XrayEndpoint
    Xray Cloud endpoint (default: xray.cloud.getxray.app)

.PARAMETER JiraProjectKey
    Jira project key (e.g. MYPROJECT)

.OUTPUTS
    exec_key - Created Test Execution key, exported to GITHUB_OUTPUT
...
# Force TLS 1.2/1.3 for compatibility with the Xray API
try {
...
# Remove orphan test_keys (tests not yet created in Jira)
